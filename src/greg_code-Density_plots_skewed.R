...
for (j in 1:nsim){print(j)
  ...
  #apply(dat_missing$amp, 2, function(x){sum(is.na(x))})
  ...
  for (i in 1:M){
    ...
    imp_dens <- data.frame(x=density(imp_data$V1, from=0, to=20, n=512)$x, y=density(imp_data$V1, from=0, to=20, n=512)$y)
    imp_dens$M <- i
  ...
  combined_dens_mean_nrd0 <- data.frame(x=density(imp_data_stacked$V1, from=0, to=20, n=512, bw = mean_nrd0)$x, y=density(imp_data_stacked$V1, from=0, to=20, n=512, bw = mean_nrd0)$y)
  ...
  #Computing density estimator with all the data and using the mean bandwidth across imputations
  combined_dens_mean_sdiqr <- data.frame(x=density(imp_data_stacked$V1, from=0, to=20, n=512, bw = stacked_bw)$x, y=density(imp_data_stacked$V1, from=0, to=20, n=512, bw = stacked_bw)$y)
  ...
  #   
  #   stat_function(aes(color = "Truth"), fun = dchisq, args = list(df = 5)) +
  #   scale_color_manual(
  #     values = c(
  #       'Truth' = 'blue',
  #       'Original' = 'red',
  ...
  orig_y <- density(dat$V1, from=0, to=20, n=512)$y # red
  missing_y <- density(na.omit(dat_missing$V1), from=0, to=20, n=512)$y # gold
  imp_stacked_y <- density(imp_data_stacked$V1, from=0, to=20, n=512, bw=stacked_bw)$y # gray
  imp_avg_bw_y <- combined_dens_mean_sdiqr$y # purple
  imp_grouped_y <- combined_dens_mean_dens$y #black
  
  orig_x <- density(dat$V1, from=0, to=20, n=512)$x
  truth_y <- dchisq(orig_x, 5) # BLUE
...
ggplot(aes(x = value, color = name), data = rezzies) + geom_density()
