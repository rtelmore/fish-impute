...
for (method in c("norm", "pmm")) {
  for (n in c(30, 100, 200, 300, 400, 500)) {
    print(paste0("n=", n))
    for (M in c(5, 10, 25, 50, 100)) {
      print(paste0("m=", M))
      for (j in 1:nsim) {
        print(j)
        # create dataset
        ...
        cop <- rCopula(
          n,
          copula = normalCopula(c(rho), dim = 2, dispstr = "un")
        )

        # Example: A mixture of two normal distributions
        mu <- c(1, 5) # Means
        sigma <- c(1, 2) # Standard deviations
        w <- c(0.3, 0.7) # Mixing proportions (should sum to 1)
        #Define the mixture
        mix <- norMix(mu = mu, sigma = sigma, w = w)
        # Example: Calculate quantiles for probabilities 0.1, 0.5, and 0.9
        dat <- data.frame(V1 = qnorMix(cop[, 1], mix), V2 = qnorm(cop[, 2]))
        ...
        for (i in 1:M) {
          ...
          imp_dens <- data.frame(
            x = density(imp_data$V1, from = -3, to = 3, n = 512)$x,
        ...
        combined_dens1 <- imp_dens_stacked %>%
          group_by(x) %>%
          summarize(y = mean(y))
        ...
        min_for_nrd0 <- imp_data_stacked %>%
          group_by(M) %>%
          summarize(nrd0fn = bw.nrd0(V1), sd = sd(V1), iqr = IQR(V1)) %>%
          ungroup() %>%
          summarize(
            sd = mean(sd),
            iqr = mean(iqr),
        ...
        combined_dens5 <- data.frame(
          x = density(imp_data_stacked$V1, from = -3, to = 3, n = 512)$x,
          y = density(imp_data_stacked$V1, from = -3, to = 3, n = 512)$y
        )
        ...
        orig <- sum(
          abs(((truth_y * diff(orig_x)[1]) - (orig_y * diff(orig_x)[1])))^2
        ) #black
        missing <- sum(
          abs(((truth_y * diff(orig_x)[1]) - (missing_y * diff(orig_x)[1])))^2
        )
        imp1 <- sum(
          abs(
            ((truth_y * diff(orig_x)[1]) - (combined_dens1$y * diff(orig_x)[1]))
          )^2
        )
        imp2 <- sum(
          abs(
            ((truth_y * diff(orig_x)[1]) - (combined_dens2$y * diff(orig_x)[1]))
          )^2
        )
        imp3 <- sum(
          abs(
            ((truth_y * diff(orig_x)[1]) - (combined_dens3$y * diff(orig_x)[1]))
          )^2
        )
        imp4 <- sum(
          abs(
            ((truth_y * diff(orig_x)[1]) - (combined_dens4$y * diff(orig_x)[1]))
          )^2
        )
        imp5 <- sum(
          abs(
            ((truth_y * diff(orig_x)[1]) - (combined_dens5$y * diff(orig_x)[1]))
          )^2
        )
      ...
      results_df <- bind_rows(
        ...
        as.data.frame(do.call(rbind, results_list)) %>%
          ...
          mutate(M = M, n = n, missing_type = missing_type, method = method)
      )
    }
  }
}
...
save(
  results_df,
  file = "/Users/maynorman/Desktop/fish_impute_git/src/results_mar_mixture_0.5.RData"
)
runtime <- end - start
runtime
...
results_df %>%
  ...
  group_by(name, M, n, method, missing_type) %>%
  summarize(-log(mean(value))) %>%
  View()
results_df %>%
  mutate(name = substring(name, 1, 4)) %>%
  ggplot(aes(x = as.factor(name), y = -log(value), color = as.factor(n))) +
  geom_boxplot() +
  facet_grid(M ~ method, scales = "free_y")

rezzies <- as.data.frame(do.call(rbind, results_list)) %>%
  pivot_longer(cols = everything())
ggplot(aes(x = (value), color = name), data = rezzies) +
  geom_density() +
  ggtitle(paste0("M = ", M)) +
  theme_bw()
...
for (m in 1:M) {
  ...
  for (i in 1:nboots) {
    ...
    boot_dens <- data.frame(
      x = density(boot$V1, from = -4, to = 4, n = 512)$x,
      y = density(boot$V1, from = -4, to = 4, n = 512)$y
    )
    boot_dens$i <- i
...
polygon(
  c(vars$x, rev(vars$x)),
  c(vars$q - 1.96 * sqrt(vars$Tm), rev(vars$q) + 1.96 * rev(sqrt(vars$Tm))),
  col = "blue",
  border = "blue"
)
