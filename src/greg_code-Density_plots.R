...
#Sample sizes between 100 and 500 at what point combined gets better than CCA 
#Correlation; 0.25, 0.5, 0.75
#MCAR (different rates of missingness in MCAR), MAR (different rates of missingness in MAR) 
...
for (n in c(30, 100, 500)){print(paste0("n=",n))
for (M in c(5, 25, 100)){print(paste0("m=",m))
...
imp_dens <- data.frame(x=density(imp_data$V1, from=-3, to=3, n=512)$x, y=density(imp_data$V1, from=-3, to=3, n=512)$y)
imp_dens$M <- i
...
orig <- sum(abs(((truth_y*diff(orig_x)[1]) - (orig_y*diff(orig_x)[1])))^2)
