...
for (n in c(30, 100, 200, 300, 400, 500)){print(paste0("n=",n))
  for (j in 1:nsim){print(j)
    ...
    for (method in c("norm", "pmm")) {
      # Do the maximum imputations once (100)
      imputed <- mice(dat_missing, print = FALSE, m = 100, method = method, seed = 12)
      imp_data_stacked <- complete(imputed, "long")
      
      for (M in c(5, 10, 25, 50, 100)) {
        print(paste0("m=", M))
        
        # take only the first M imputations
        imp_data_subset <- imp_data_stacked[imp_data_stacked$.imp %in% 1:M, ]
        
        imp_dens_stacked <- data.frame()
        for (i in 1:M) {
          imp_dens <- data.frame(
            x = density(imp_data_subset$V1[imp_data_subset$.imp == i],
                        from = -3, to = 3, n = 512)$x,
            y = density(imp_data_subset$V1[imp_data_subset$.imp == i],
                        from = -3, to = 3, n = 512)$y
          )
          imp_dens$M <- i
        ...
        # Combine across imputations (Imp1 … ImpM)
        combined_dens1 <- imp_dens_stacked %>%
          group_by(x) %>%
          summarize(y = mean(y), .groups = "drop")
        
        # Save results
        results_list[[paste(method, M, n, j, sep = "_")]] <- combined_dens1
      }
    }
