    ...
    for (method in c("norm", "pmm")) {
      ...
      for (M in mvec) {
        ...
        for (i in 1:M) {
          ...
          imp_dens <- data.frame(
            x = density(
              imp_data_stacked$V1[imp_data_stacked$M == i],
              from = -3,
              to = 18,
              n = n_interval
            )$x,
        ...
        combined_dens2 <- data.frame(
          x = density(
            imp_data_stacked$V1[imp_data_stacked$M <= M],
            from = -3,
            to = 18,
            n = n_interval,
            bw = mean_nrd0
        ...
        ggplot() +
          ...
          geom_path(
            aes(
              ...
              y = y,
              group = as.factor(M),
              color = "Individual Imputation"
            ),
            data = imp_dens_stacked
          ) +
          theme_bw() +
          geom_path(
            aes(x = x, y = y, color = "Imputed Combined: 1"),
            data = combined_dens1
          ) +
          geom_path(
            aes(x = x, y = y, color = "Imputed Combined: 2"),
            data = combined_dens2
          ) +
          geom_path(
            aes(x = x, y = y, color = "Imputed Combined: 3"),
            data = combined_dens3
          ) +
          geom_path(
            aes(x = x, y = y, color = "Imputed Combined: 4"),
            data = combined_dens4
          ) +
          stat_function(
            aes(color = "Truth"),
...
for (m in 1:M) {
  ...
  for (i in 1:nboots) {
    ...
    boot_dens <- data.frame(
      x = density(boot$V1, from = -4, to = 4, n = n_interval)$x,
      y = density(boot$V1, from = -4, to = 4, n = n_interval)$y
    )
    boot_dens$i <- i
