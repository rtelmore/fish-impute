...
#chisq results flipped
load("../fish_impute_git/src/results_mar_chisq_0.25_reversemissing.RData") 
resultsdf25chiflip = results_df %>%
  mutate(corr = .25)
load("../fish_impute_git/src/results_mar_chisq_0.5_reversemissing.RData")
resultsdf50chiflip = results_df %>%
  mutate(corr = .50)

combin_df = bind_rows(resultsdf25chiflip, resultsdf50chiflip)
#plot chi sq flipped results
#ggplot(combin_df,facet_grid(rows = corr, cols = M))
combin_df %>%filter(name %in% c("Original","Missing","Imp1")) %>% mutate(name = substring(name,1,4)) %>% ggplot(aes(x = as.factor(name), y = -log(value), color = as.factor(M))) + geom_boxplot() + facet_grid(corr~n+method)
